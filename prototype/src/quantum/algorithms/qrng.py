...
def _generate_random_bits(n: int, device: qdev.QuantumDevice) -> list[int]:
    assert n <= device.n_available_qubits()

    with device.n_alloc(n) as qubits:
        return [_qubit_to_bit(state) for _,state in
...
def qrng(
    ...
    while i < n:
        ...
        result.extend(_generate_random_bits(batch_size, device))
        i += batch_size
    return result
