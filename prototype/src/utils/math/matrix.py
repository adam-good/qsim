...
class Matrix:
    ...
    def col_vectors(self) -> Tuple[Vector, ...]:
        return tuple(Vector(c) for c in
            zip(*self.raw_data) # This acts as the transpose
        )

    def row_vectors(self) -> Tuple[Vector, ...]:
        return tuple(Vector(r) for r in self.raw_data)
    ...
    def __matmul__(self, other: Matrix | Vector) -> Matrix | Vector:
        if isinstance(other, Matrix):
            ...
            return Vector(tuple(
                Vector.dotprod(row, other) for row in self.row_vectors()
            ))
        return NotImplemented
